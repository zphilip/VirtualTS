...
//====================
//ちょっと変更
unsigned int cTextureList::getTexture( const char *szDirFilePath, const char *szFileName )
{
	...
	strcpy(fullPath, szDirFilePath);
	strcat(fullPath, szFileName);

	// なければファイルを読み込んでテクスチャ作成
	if( createTexture( fullPath, &uiTexID ) )
	{
		// 新しいリストノードを作成し先頭へ接続
...
void cTextureList::debugDraw( void )
{
	...
	while( pTemp )
	{
		...
		glBegin( GL_TRIANGLE_FAN );
			glVertex3f( fPosX,             fPosY - DISP_SIZE, 0.0f );	glTexCoord2f( 1.0f, 1.0f );
			glVertex3f( fPosX + DISP_SIZE, fPosY - DISP_SIZE, 0.0f );	glTexCoord2f( 1.0f, 0.0f );
			glVertex3f( fPosX + DISP_SIZE, fPosY,             0.0f );	glTexCoord2f( 0.0f, 0.0f );
			glVertex3f( fPosX,             fPosY,             0.0f );	glTexCoord2f( 0.0f, 1.0f );
		glEnd();
