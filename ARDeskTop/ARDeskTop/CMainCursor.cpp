...
int CMainCursor::Click(void)
{
	...
	if(selectObjectIndex == -1) { //何もないところでクリック
		...
		} else if(CUser::cursorMode == CUser::kGame && CARInvaders::arInvaders) { //ミサイル
			CDesktop::desktop->ProjectCursorPos(this, missileStartPos);
			CDesktop::desktop->world->GlobalToLocal(missileStartPos);

			CMissile* missile = new CMissile();
			double m[16];

			argConvGlpara(CDesktop::desktop->invMat, missile->matrix);
			argConvGlpara(trans, m);
			missile->Multiply(m);

		//	ConvertToARMat(CDesktop::desktop->world->matrix, mat);
		//	arUtilMatInv(mat, matrixInv);
		//	argConvGlpara(CDesktop::desktop->world->matrixInv, m);
		//	cube->Multiply(m);

			missile->Rotate(-90.0, 1.0, 0.0, 0.0);
			missile->Scale(5.0);
			missile->SetPosition(missileStartPos);

			missile->Init();
			CARInvaders::arInvaders->AddMissile(missile);
			ARDTPlaySound(L"Data/missile.wav");
		}
	} else {
