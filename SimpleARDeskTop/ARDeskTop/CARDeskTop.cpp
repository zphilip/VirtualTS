...
int CARDeskTop::mode = kVR; //kDisplayCalibration
...
int CARDeskTop::ReadData(char* filename)
{
	...
	if(CCursor::projectionNum)
		CCursor::projectionList	= new CProjection*[CCursor::projectionNum];
	if(CCursor::cursorNum)
		CCursor::cursorList		= new CCursor*[CCursor::cursorNum];
	if(CCursor::buttonNum)
		CCursor::buttonList		= new CMarkerButton*[CCursor::buttonNum];
	...
	if(mode == kDisplayCalibration) objectnum++;
	...
	for(i = 0; i < objectnum; i++) {
		...
		if(strcmp("CDesktop", className) == 0) {
			...
			if(mode == kVR || mode == kVR2 || mode == kiPadVR) {
				doNotScan	= true;
				isVirtual	= true;
			}
			...
			CAccViewer*	accViewer = new CAccViewer();
			object = (CMarkerObject*) accViewer;
			CCursor::projectionList[p++] = (CProjection*) accViewer;
...
void CARDeskTop::Run(int argc, char** argv, int pmode, char* filename)
{
	...
	if(mode == kVR) {
		CWall::wall	= new CWall2();
		CWall::wall->Init();
	} else  if(mode == kVR2) {
		CWall::wall	= new CWall3();
		CWall::wall->Init();
	}
